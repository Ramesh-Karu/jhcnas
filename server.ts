...
// 8. Live Supabase Upsert Records (with 250-row chunking and rich diagnostics)
app.post('/api/supabase/upsert-records', async (req, res) => {
  try {
    ...
    // Only apply onConflict if column has values
    ...
    for (let i = 0; i < cleanedRecords.length; i += BATCH_SIZE) {
      ...
      if (!upsertRes.ok) {
        ...
        // Fallback: Retry with direct INSERT if on_conflict is not a unique constraint
        if (effectiveConflictCol || errText.includes('ON CONFLICT') || errText.includes('constraint') || errText.includes('42P10') || upsertRes.status === 400) {
          ...
          if (fallbackRes.ok) {
            ...
            if (Array.isArray(data)) allInsertedRecords.push(...data);
            continue;
          } else {
        ...
        if (errText.includes('invalid input syntax for type integer') || errText.includes('22P02')) {
          ...
          if (candidateCols.size > 0) {
            ...
            const healUrl = `${cleanUrl}/rest/v1/${encodeURIComponent(tableName)}`;
            ...
            if (healRes.ok) {
              ...
              totalUpserted += healedBatch.length;
              if (Array.isArray(data)) allInsertedRecords.push(...data);
              continue;
            }
          }
        ...
        const diagnostic = diagnoseSupabaseError(upsertRes.status, errText, tableName, batch[0]);
        firstError = {
    ...
    if (firstError) {
      return res.status(firstError.status).json({
        ...
        error: `Supabase upsert failed for table '${tableName}': ${firstError.errorText}`,
    ...
    return res.json({
      ...
      upsertedCount: totalUpserted,
      totalRecords: cleanedRecords.length,
      records: allInsertedRecords,
      message: `Successfully pushed ${totalUpserted} rows to public.${tableName}!`,
    });
  } catch (error: any) {
    return res.status(500).json({ success: false, error: error.message });
  }
...
<d:propfind xmlns:d="DAV:">
  ...
  for (const wm of activeMappings) {
    ...
    // Execute upsert into Supabase PostgREST in 250-row chunks for high reliability with large datasets (3000+ rows)
    const BATCH_SIZE = 250;
    let tableInserted = 0;
    let tableFailed = 0;
      ...
      if (!upsertRes.ok) {
        ...
        if (!upsertRes.ok && (errText.includes('invalid input syntax for type integer') || errText.includes('22P02'))) {
          ...
          if (candidateCols.size > 0) {
            ...
            const healedRecords = batch.map((r: any) => {
              const copy = { ...r };
              for (const col of candidateCols) {
                if (copy[col] !== undefined && copy[col] !== null) {
                  const s = String(copy[col]).trim();
        ...
        if (!upsertRes.ok) {
          ...
          const count = Array.isArray(upsertedData) ? upsertedData.length : batch.length;
          tableInserted += count;
        }
      } else {
        const upsertedData = await upsertRes.json().catch(() => []);
    ...
    if (lastTableError) {
      errors.push(lastTableError);
      syncResults.push({
        sheetName: wm.worksheetName,
        targetTable,
        rowsCount: recordsToUpsert.length,
        insertedCount: tableInserted,
        failedCount: tableFailed,
        status: 'Failed',
        error: lastTableError.error,
        diagnostic: lastTableError.diagnostic,
      });
    } else {
      syncResults.push({
        sheetName: wm.worksheetName,
        targetTable,
        rowsCount: tableInserted,
        insertedCount: tableInserted,
        uniqueKey: uniqueCol,
        status: 'Success',
      });
