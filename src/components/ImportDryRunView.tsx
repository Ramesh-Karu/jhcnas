...
export const ImportDryRunView: React.FC<ImportDryRunViewProps> = ({
  ...
  const currentFilename = currentAnalysis?.filename || files[0]?.filename || '';
  const currentFileIndex = files.findIndex(f => f.filename.toLowerCase().trim() === currentFilename.toLowerCase().trim());

  const handleNextFile = () => {
    if (files.length === 0) return;
    const nextIdx = (currentFileIndex + 1) % files.length;
    handleSelectNextcloudFile(files[nextIdx].filename);
  };

  const handlePrevFile = () => {
    if (files.length === 0) return;
    const prevIdx = (currentFileIndex - 1 + files.length) % files.length;
    handleSelectNextcloudFile(files[prevIdx].filename);
  };
      ...
      <div className="bg-white rounded-xl p-6 border border-slate-200 shadow-2xs flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">
              Simulate transformations, unique key conflict resolution, and schema validation for individual Excel files without modifying live data.
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center space-x-3">
          <button
            ...
            className="inline-flex items-center space-x-2 px-4 py-2 rounded-lg border border-slate-300 bg-white hover:bg-slate-50 text-xs font-semibold text-slate-700 shadow-2xs transition-colors cursor-pointer"
            title={`Re-run dry run simulation for ${currentFilename}`}
          >
            <RefreshCw className={`w-3.5 h-3.5 text-slate-500 ${isRunningDryRun || isLoadingFile ? 'animate-spin' : ''}`} />
            <span>{isRunningDryRun ? 'Simulating...' : `Dry Run ${currentFilename ? `(${currentFilename})` : ''}`}</span>
          </button>
          ...
          <button
            ...
            title={`Write validated rows from ${currentFilename} to Supabase`}
          >
            <Play className={`w-3.5 h-3.5 ${isImporting ? 'animate-spin' : ''}`} />
            <span>{isImporting ? 'Writing to Supabase...' : `Import ${currentFilename ? `(${currentFilename})` : ''}`}</span>
          </button>
        </div>
      </div>

      {/* Individual File Selector Carousel */}
      {files && files.length > 0 && (
        <div className="bg-white rounded-xl p-4 border border-slate-200 shadow-2xs space-y-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-2">
              ...
              <span className="text-xs font-bold text-slate-800 uppercase tracking-wider">
                Select Individual File to Dry Run ({files.length} Available):
              </span>
            </div>
            ...
            <div className="flex items-center space-x-1.5">
              <button
                ...
                disabled={isLoadingFile || files.length <= 1}
                className="inline-flex items-center space-x-1 px-2.5 py-1 rounded-md border border-slate-300 bg-white hover:bg-slate-50 text-xs font-semibold text-slate-700 transition-colors disabled:opacity-40 cursor-pointer"
                title="Simulate previous file"
              >
          ...
          <div className="flex items-center gap-2 overflow-x-auto pb-1 pt-0.5">
            {files.map((file, fIdx) => {
              const isSelected = file.filename.toLowerCase().trim() === currentFilename.toLowerCase().trim();
              return (
                <button
                  key={`${file.id || file.filename}-${fIdx}`}
                  type="button"
                  onClick={() => handleSelectNextcloudFile(file.filename)}
                  disabled={isLoadingFile}
                  className={`shrink-0 inline-flex items-center space-x-2 px-3 py-2 rounded-lg text-xs font-medium border transition-all cursor-pointer ${
                    isSelected
                      ? 'bg-indigo-600 text-white border-indigo-700 shadow-xs ring-2 ring-indigo-200 font-bold'
                      : 'bg-slate-50 hover:bg-slate-100 text-slate-700 border-slate-200 hover:border-slate-300'
                  }`}
                  title={`Click to dry run ${file.filename} individually`}
                >
      ...
      <div className="bg-white rounded-xl p-4 border border-slate-200 shadow-2xs flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center space-x-2">
          ...
          <button
            ...
            className="inline-flex items-center space-x-1 px-2.5 py-1.5 rounded-lg text-xs font-semibold text-emerald-700 hover:text-emerald-800 hover:bg-emerald-50 transition-colors cursor-pointer"
          >
